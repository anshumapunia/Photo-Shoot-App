...
async function recentDocs() {
  try {
      let res = await fetch(baseURL + "photographer");
      if (res.ok) {
          let data = await res.json();
          // console.log(data);
...
async function recentApps() {
  try {
let res = await fetch(baseURL+"appointment/allappoinment");
      if (res.ok) {
          let data = await res.json();
...
async function gettrainers() {
  ...
  try {
      let res = await fetch(baseURL + `user/singlephotographer/${id}`, {
          method: "GET",
          headers: {
...
async function updatetrainer(event) {
  ...
  try {
      ...
      swal("", `Photographer Detail updated`, "success")
          .then(function () {
              recentDocs();
          });
  }
